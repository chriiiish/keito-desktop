 ...
 */
export function TodayList({ entries, catalog, busyId, onResume }: TodayListProps): JSX.Element {
          ...
          {named.map(({ entry, taskName, projectName }) => (
            <li key={entry.id} className={entry.is_running ? "running-row" : ""}>
              ...
              <button
                ...
                title={entry.is_running ? "Already running" : "Resume this entry"}
                aria-label={`Resume ${taskName}`}
                disabled={entry.is_running || busyId !== null}
